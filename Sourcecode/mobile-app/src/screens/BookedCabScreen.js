...
import { appConsts, MAIN_COLOR, SECONDORY_COLOR, getRandomId } from '../common/sharedFunctions';
import { Ionicons } from '@expo/vector-icons';
import { fonts } from '../common/font';
import { editInventario } from 'common/src/actions/inventarioactions';
import ActionSheet from 'react-native-actions-sheet';
import MaterialIcon from 'react-native-vector-icons/MaterialIcons'; // Para el ícono de documento
...
export default function BookedCabScreen(props) {
    ...
    const [uploadOptionsModalShow, setUploadOptionsModalShow] = useState(false); //
                    ...
                    {role == 'driver' && (!appConsts.captureBookingImage || (curBooking.deliver_image && appConsts.captureBookingImage) || (settings && !settings.AllowFinalDeliveryImageCapture && appConsts.captureBookingImage)) && curBooking.status == 'STARTED' ?
                        <View style={{ flex: 1 }}>
                            <Button
                                ...
                                onPress={() => {
                                    console.log("SOM: ", curBooking.otp && !appConsts.hasStartOtp)
                                    if (curBooking.otp && !appConsts.hasStartOtp) {
                                        setOtpModalVisible(true);
                                    } else {
                                        endBooking();
    ...
    const endBooking = async () => {
        ...
        //         'Para finalizar el trabajo, primero debes adjuntar las evidencias.',
        //         [{ text: 'Entendido' }]
        //     );
        //     return;
        // }
        ...
        for(let i = 0; i < patios.length; i++) {
            ...
            if (patio.patio_lat == booking.drop.lat && patio.patio_lng == booking.drop.lng) {
                curPatio = patio;
                break;
            }
        }
        ...
        let carTypeIndex = `cupo_${carType.toLowerCase()}s`;
        curPatio[carTypeIndex] = curPatio[carTypeIndex] - 1;

        let inventarios = inventariosdata.inventarios;
        // [{"id": "QSgFawsQEsCSaERTwWeeA4GFERd8", "inventory": [[Object]], "patio_id": "GUgZzoaAWsDSaLGJwQeeZ4EIZIr2"}]

        let curInventory = null;
        if (inventarios.length === 0) {
            console.log('El array de inventarios está vacío');
        } else {
            console.log('Inventarios encontrados:', inventarios);
        }
        for (let i = 0; i < inventarios.length; i++) {
            curInventory = inventarios[i];
            // Verificamos los valores que se comparan
            console.log(`Comparando curInventory.patio_id: ${curInventory.patio_id} con curPatio.id: ${curPatio.id}`);
            if (curInventory.patio_id == curPatio.id) {
                console.log('Patio coincidente encontrado, id:', curPatio.id);
                // Aseguramos que curInventory.inventory sea un array
                if (!Array.isArray(curInventory.inventory)) {
                    console.log('El campo inventory no existe o no es un array, inicializando como un array vacío.');
                    curInventory.inventory = []; // Inicializamos como un array vacío
                }
                const newId = getRandomId(curInventory.inventory);
                console.log('Nuevo ID generado:', newId);
                // Verificar la longitud inicial del inventario
                console.log('Longitud del inventario antes de agregar:', curInventory.inventory.length);
                curInventory.inventory.push({
                    "id": newId,
                    "patio_id": curPatio.id,
                    "type": carType,
                    "description": "Descripción de prueba",
                    "approved": false,
                });
                // Verificar la longitud del inventario después de agregar
                console.log('Nuevo elemento añadido, longitud del inventario después de agregar:', curInventory.inventory.length);
                console.log('Nuevo inventario añadido:', curInventory.inventory);
                break;
            }
        }

        if (!curInventory) {
            console.log('No se encontró un inventario con el patio_id proporcionado');
        } else {
            console.log('Inventario final después de la modificación:', curInventory);
        }


        // Sacamos el último elemento agregado de curInventory.inventory
        const lastItem = curInventory.inventory[curInventory.inventory.length - 1];
        // Llamamos a handleSendEvidence pasándole el último item junto con el patio
        const result = await handleSendEvidence(curPatio, lastItem);
        // Agregamos las evidencias al último item dentro del curInventory
        if (result) {
            curInventory.inventory[curInventory.inventory.length - 1].evidences = result; // Agrega la nueva propiedad 'evidences'
        }

        setTimeout(() => {
            dispatch(editPatio(curPatio, "Update"));
        }, 1000);
        setTimeout(() => {
            if (curInventory) {
                dispatch(editInventario(curInventory, "Update"));
            }
        }, 2000);
    ...
    const handleFileUpload = (fileBlob) => {
        if (fileBlob) {
            setSelectedFile(fileBlob);  // Guardar el archivo correctamente en el estado
        } else {
            console.log('Error: El archivo seleccionado no tiene datos.');
        }
    };

    const handleImageUpload = (imageUri) => {
        if (imageUri) {
            setImageList((prevList) => [...prevList, imageUri]);  // Guardar la imagen en la lista de imágenes
        } else {
            console.log('Error: La imagen seleccionada no tiene URI.');
        }
    };
    ...
    const handleOpenUploadOptionsModal = () => {
        setUploadOptionsModalShow(true);
    };

    const handleGetTemplateDocument = async () => {
        const url = await getTemplateDocument();
        console.log(url);
    ...
    const _pickDocumentImage = async (sourceType) => {
        try {
            let result;
            ...
            if (result && !result.canceled && result.assets && result.assets.length > 0) {
                ...
                if (sourceType === 'document') {
                    ...
                    handleFileUpload(documentBlob);  // Guardar el archivo correctamente
                } else {
                    const imageBlob = await response.blob();
                    handleImageUpload(uri);  // Guardar la imagen en la lista de imágenes
                }
            } else {
                console.log("Selección de archivo cancelada o sin resultados.");
            }
        } catch (error) {
            console.error('Error al seleccionar documento o imagen:', error);
        }
    };

    const handleSendEvidence = async (curPatio, curInventory) => {
        // setSpinnerLoading(true);
        try {
            //----
            const uploadPromises = [];
            if (selectedFile) {
                const mimeType = selectedFile._data.type;
                ...
                const fileExtension = mimeToExtensionMap[mimeType] || 'unknown';
                const fileName = `PATIOSYGRUAS_ORIGINAL_Inventario_Autos_y_Motos_${Date.now()}.${fileExtension}`;
                uploadPromises.push(updateEvidenceDocument(curPatio.id, curInventory.id, selectedFile, fileName));
            }
            ...
            if (imageList.length > 0) {
                ...
                const uploadPromise = fetch(imageUri)
                    .then((response) => response.blob())
                    .then((imageBlob) => updateEvidencePhoto(curPatio.id, curInventory.id, imageBlob, fileName));
            ...
            console.log('Todas las evidencias se subieron correctamente:', results);
            return results;

            // setEvidences((prevEvidences) => [...prevEvidences, ...results]);
            // Alert.alert('Éxito', 'Todas las evidencias se han subido correctamente.');
            ...
            return [];
            // Alert.alert('Error', 'Ocurrió un error al subir las evidencias.');
        } finally {
    ...
    return (
        <View style={styles.mainContainer}>
            <View style={styles.mapcontainer}>
                ...
                {/* Boton de Evidencia */}
                {(role === 'driver') && // Aseguramos que el rol sea "driver"
                    (curBooking && 
                    curBooking.status && 
                    auth.profile && 
                    auth.profile.uid && 
                    (['STARTED'].indexOf(curBooking.status) != -1)) && 
                    (settings && settings.panic && settings.panic.length > 0) &&
                    (appConsts.canCall) ? // Eliminamos la condición de rol en esta parte
                        <TouchableOpacity 
                            style={[styles.floatButton, isRTL ? { right: 10, bottom: (role == 'customer' && ((curBooking && curBooking.status == 'ARRIVED') || curBooking.status == 'REACHED' || curBooking.status == 'PAID' || (curBooking && curBooking.status == 'STARTED'))) ? 343 : 418 } : { left: 10, bottom: (role == 'customer' && ((curBooking && curBooking.status == 'ARRIVED') || curBooking.status == 'REACHED' || curBooking.status == 'PAID' || (curBooking && curBooking.status == 'STARTED'))) ? 358 : (role == 'customer' && (curBooking && curBooking.status == 'ACCEPTED')) ? 408 : 388 }]}
                            onPress={() => handleOpenUploadEvidenceModal(curBooking)} >
                            <Icon
                                name="upload"
                                type="feather"
                                size={30}
            ...
            <EvidenceModal
                ...
                onCloseModal={() => setEvidenceModalShow(false)}
                selectedFile={selectedFile}
                imageList={imageList}
                spinnerLoading={loading}
                onGetTemplateDocument={handleGetTemplateDocument}
                onOpenUploadOptionsModal={handleOpenUploadOptionsModal}
                onSendEvidence={endBooking}
                onRemoveImage={handleRemoveImage}
                onRemoveFile={handleRemoveFile}
            />
            {/* ---- */}
            <UploadOptionsModal
                isVisible={uploadOptionsModalShow}
                onClose={() => setUploadOptionsModalShow(false)}
                _pickDocumentImage={_pickDocumentImage}
            />
        </View>
    );
...
/** ################################################################################ */
const EvidenceModal = ({
    evidenceModalShow = false,
    onCloseModal,
    selectedFile,
    imageList,
    spinnerLoading,
    onGetTemplateDocument,
    onOpenUploadOptionsModal,
    onSendEvidence,
    onRemoveImage,
    onRemoveFile
}) => {
    return (
        <Modal visible={evidenceModalShow} animationType="slide">
            {spinnerLoading && (
                <View style={{
                    position: 'absolute',
                    top: 0,
            ...
            <View style={{ flex: 1, justifyContent: 'space-between' }}>
                ...
                <View style={{ padding: 10, flex: 1 }}>
                    {/* ---- */}
                    <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Plantilla o Formato</Text>
                    {/* Enlace para descargar el documento */}
                    <TouchableOpacity onPress={onGetTemplateDocument} style={{ marginVertical: 10 }}>
                        <Text style={{ color: '#1E90FF', textDecorationLine: 'underline' }}>
                            Descargar plantilla de Inventario Autos y Motos
                        </Text>
                    </TouchableOpacity>
                    {selectedFile ? (
                        <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 10 }}>
                            <Ionicons name="document" size={48} color="black"/>
                            <Text style={{ marginLeft: 5 }}>Archivo cargado</Text>
                            <TouchableOpacity onPress={onRemoveFile} style={{ marginLeft: 10 }}>
                                <Ionicons name="trash" size={24} color="red" />
                            </TouchableOpacity>
                        </View>
                    ) : (
                        <Text>No se ha cargado ningún archivo</Text>
                    )}
                    {/* ---- */}
                    <View style={{ height: 1, backgroundColor: '#cccccc', marginVertical: 15 }} />
                    {/* ---- */}
                    <Text style={{ fontSize: 18, fontWeight: 'bold' }}>Listado de Imágenes</Text>
                    {imageList.length > 0 ? (
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', padding: 10 }}>
                            {imageList.map((imageUri, index) => (
                                <View key={index} style={{ position: 'relative', marginRight: 10, marginBottom: 10 }}>
                                    <Image
                                        source={{ uri: imageUri }}
                                        style={{ borderRadius: 5, width: 110, height: 110 }}
                                    />
                                    <TouchableOpacity
                                        onPress={() => onRemoveImage(index)}
                                        style={{ position: 'absolute', backgroundColor: '#ffffff', borderRadius: 50, padding: 3, top: 5, right: 5 }}
                                    >
                                        <Ionicons name="trash" size={24} color="red" />
                                    </TouchableOpacity>
                                </View>
                        ...
                        <Text>No se han cargado imágenes</Text>
                    )}
                </View>
                {/* ---- */}
                <View style={{ padding: 10 }}>
                    <TouchableOpacity style={{
                        ...
                        marginBottom: 15,
                        alignItems: 'center',
                    }} onPress={onOpenUploadOptionsModal}>
                        <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: 16 }}>Agregar Archivo/Imagen</Text>
                    </TouchableOpacity>
                    {/* ---- */}
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                        ...
                        <TouchableOpacity
                            style={{
                                backgroundColor: selectedFile && !spinnerLoading ? MAIN_COLOR : '#A9A9A9', //
                                padding: 15,
                                borderRadius: 10,
                                flex: 1,
                                marginLeft: 5,
                            ...
                            onPress={selectedFile && !spinnerLoading ? onSendEvidence : null} //
...
const UploadOptionsModal = ({ isVisible, onClose, _pickDocumentImage }) => {
    return (
            ...
            <View style={{
                ...
                <View style={{
                    ...
                    <TouchableOpacity style={{
                        ...
                        width: '100%',
                        alignItems: 'center',
                    }} onPress={() => _pickDocumentImage('camera')}>
                        <Text style={{ color: 'white', fontSize: 16, fontWeight: 'bold' }}>Tomar Foto</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity style={{
                        ...
                        width: '100%',
                        alignItems: 'center',
                    }} onPress={() => _pickDocumentImage('gallery')}>
                        <Text style={{ color: 'white', fontSize: 16, fontWeight: 'bold' }}>Seleccionar de la Galería</Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity style={{
                        ...
                        width: '100%',
                        alignItems: 'center',
                    }} onPress={() => _pickDocumentImage('document')}>
                        <Text style={{ color: 'white', fontSize: 16, fontWeight: 'bold' }}>Subir Archivo</Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    mainContainer: { flex: 1, backgroundColor: colors.WHITE, position: 'relative' },
    headerStyle: {
