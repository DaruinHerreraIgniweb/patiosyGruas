...
export const updateVehicleInventoryDocument = (documentBlob) => {
  const {
    auth,
    vehicleInventoryRef,
  } = firebase;
  ...
  uploadBytesResumable(vehicleInventoryRef(uid), documentBlob)
    .then(() => {
      documentBlob.close(); //
      return getDownloadURL(vehicleInventoryRef(uid)); //
    })
    .then((url) => {
...
/** ################################################################################ */
export const updateEvidenceDocument = (patioUid, inventoryUid, documentBlob, fileName) => {
  const { evidenceDocumentRef } = firebase;

  return uploadBytesResumable(evidenceDocumentRef(patioUid, inventoryUid, fileName), documentBlob)
    .then(() => {
      documentBlob.close();
      return getDownloadURL(evidenceDocumentRef(patioUid, inventoryUid, fileName));
    })
    .then((url) => {
      console.log('URL del documento subido:', url);
      return url;
    })
    .catch((error) => {
      console.error('Error al subir el documento:', error);
...
export const updateEvidencePhoto = (patioUid, inventoryUid, photoBlob, fileName) => {
  const { evidencePhotoRef } = firebase;

  return uploadBytesResumable(evidencePhotoRef(patioUid, inventoryUid, fileName), photoBlob)
    .then(() => {
      photoBlob.close();
      return getDownloadURL(evidencePhotoRef(patioUid, inventoryUid, fileName));
    })
    .then((url) => {
...
export const getTemplateDocument = async () => {
  try {
    const { templateDocumentRef } = firebase;
    const url = await getDownloadURL(templateDocumentRef()); // Obtener la URL de descarga
    return url;
  } catch (error) {
