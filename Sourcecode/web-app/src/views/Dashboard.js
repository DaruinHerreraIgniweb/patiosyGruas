...
const Dashboard = () => {
    ...
    useEffect(() => {        
        let parks = [];
        let parkImage = `https://i.imgur.com/tqsXQ1K.png`;
        const patios = patiosdata.patios;
        for(let i = 0; i < patios.length; i++) {        
            parks.push({
                lat: patios[i].patio_lat,
                lng: patios[i].patio_lng,
                parkImage,
                parkingName: patios[i].patio_name,
                parkingCapacity: patios[i].patio_capacity,
                parkingAddress: patios[i].patio_address
            });
        }
        setParkLocations(parks);
    }, [patiosdata]);
        ...
        if (usersdata.drivers && bookinglistdata.bookings) {
            ...
            for (let i = 0; i < drivers.length; i++) {
                if (drivers[i].location) {
                    let carImage = 'https://i.imgur.com/5r95XGS.png';
                    let bookingRef = null;
                    for (let j = 0; j < cars.length; j++) {
                        if (cars[j].name === drivers[i].carType) {
                            carImage = cars[j].image || carImage;
                        }
                    }
    ...
    useEffect(() => {
        if (usersdata.users) {
            ...
            let activeCount = 0;
            let customerCount = 0;
            let parkingCount = patiosdata.patios.length;
    ...
    const handleToggleOpen = (id) => {
        // Si el InfoBox está abierto y se clickea de nuevo, se cierra
        if (openInfoBoxId === id) {
          setOpenInfoBoxId(null);
        } else {
          setOpenInfoBoxId(id);
        }
      };
                    ...
                    <Grid container style={{  direction: isRTL === 'rtl' ? 'rtl' : 'ltr' }} >
                        <Grid item xs={12} sm={12} md={9} lg={9} style={{ backgroundColor: colors.WHITE, borderRadius: 20, overflow: 'hidden', }} boxShadow={3}>
                            ...
                            <GoogleMap
                                ...
                                {parkLocations.map(marker => (
                                    <Marker
                                        position={{ lat: marker.lat, lng: marker.lng }}
                                        key={marker.id}
                                        icon={{
                                            url: marker.parkImage,
                                            scaledSize: new window.google.maps.Size(32, 32)
                                        }}
                                        onClick={() => handleToggleOpen(marker.id)}
                                    >
                                        {openInfoBoxId === marker.id && (
                                            <InfoBox 
                                            position={{ lat: marker.lat, lng: marker.lng }} 
                                            options={{
                                                pixelOffset: new window.google.maps.Size(-120, -110),
                                                closeBoxURL: '',
                                                enableEventPropagation: true,
                                                boxStyle: {
                                                backgroundColor: '#D4D42E',
                                                width: '240px',
                                                borderRadius: '10px',
                                                padding: '10px',
                                                color: 'black',
                                            ...
                                            <div style={{ position: 'relative', fontFamily: 'Arial' }}>
                                                ...
                                                {marker.parkingName}<br/>
                                                Capacidad: {marker.parkingCapacity}<br/>
                                                {marker.parkingAddress}
                                            </div>
                                            </InfoBox>
                                        )}
                                    </Marker>                                    
                        ...
                        <Grid item xs={12} sm={12} md={3} lg={3} style={{ padding: 25, marginTop: '30px' }}>
                            ...
                            <Card style={{ borderRadius: "19px", backgroundColor: colors.GREEN, minHeight: 100, marginBottom: 20 }}>
                            ...
                            <Card style={{ borderRadius: "19px", backgroundColor: "#D4D42E", minHeight: 100 }}>
                                <Typography style={{ color: 'white', textAlign: 'center', fontSize: 20, marginTop: 20, fontFamily: FONT_FAMILY }}>{t('total_parkings')}</Typography>
                                <Typography style={{ color: 'white', textAlign: 'center', fontSize: 26, fontWeight: 'bold', fontFamily: FONT_FAMILY }}>{parkingCount}</Typography>
                            </Card>
                        </Grid>
                    </Grid>
                </Grid>
